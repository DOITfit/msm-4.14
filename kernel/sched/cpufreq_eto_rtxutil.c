 ...
 * @sg_policy: eto_rtxutil policy object to compute the new frequency for.
...
static void smugov_irq_work(struct irq_work *irq_work)
{
	...
	/*
	 * For RT and deadline tasks, the eto_rtxutil governor shoots the
	 * frequency to maximum. Special care must be taken to ensure that this
	 * kthread doesn't result in the same behavior.
	 *
	 * This is (mostly) guaranteed by the work_in_progress flag. The flag is
	 * updated only at the end of the smugov_work() function and before that
	 * the eto_rtxutil governor rejects all other frequency scaling requests.
	 *
...
static struct cpufreq_governor eto_rtxutil_gov;
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_ETO_RTXUTIL
struct cpufreq_governor *cpufreq_default_governor(void)
{
	return &eto_rtxutil_gov;
}
#endif

static int __init smugov_register(void)
{
	return cpufreq_register_governor(&eto_rtxutil_gov);
}
